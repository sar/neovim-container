...
# >= 0.7.2, 0.8.2
ARG RELEASE_NVIM=0.7.2
ARG RELEASE_BIN_NERDFONTS=2.1.0
# 2.0.0-880 <= net6
...
# --------------
WORKDIR /tmp
RUN curl -LO https://github.com/neovim/neovim/releases/download/v${RELEASE_NVIM}/nvim-linux64.tar.gz && \
    tar -xvf nvim-linux64.tar.gz && \
    mv nvim-linux64/bin/* /usr/local/sbin/ && \
    mv nvim-linux64/lib/* /usr/lib/ && \
    mv nvim-linux64/share/* /usr/share/

# nerdfonts
RUN wget https://github.com/ryanoasis/nerd-fonts/releases/download/v${RELEASE_BIN_NERDFONTS}/DroidSansMono.zip && \
...
# SDK: Mono 
# --------------
#apt-key adv --keyserver hkps://keyserver.ubuntu.com:443 --recv-keys 3FA7E0328081BFF6A14DA29AA6A19B38D3D831EF && \
#RUN  echo 1 && \ 
...
# NPM: Packages
# --------------
#RUN chown $DEFAULT_USER /usr/lib/node_modules
#USER $DEFAULT_USER
USER root
...
# BUG: /var/run/docker.sock not avialable before mount
# runtime chown $DEFAULT_USER /var/run/docker.sock using init.sh

# export binaries
USER $DEFAULT_USER
RUN export PATH="~/.dotnet/tools:$PATH" && \
    export PATH="/tmp/gitui:$PATH" && \
    export PATH="/tmp/bin:$PATH" && \
    export PATH="/tmp/netcoredbg:$PATH"
